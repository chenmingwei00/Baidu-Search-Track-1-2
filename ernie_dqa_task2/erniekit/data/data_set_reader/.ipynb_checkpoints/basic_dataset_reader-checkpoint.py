...
class BasicDataSetReader(BaseDataSetReader):
    ...
    def __iter__(self):
        ...
        for epoch_index in range(self.config.epoch):
            ...
            for input_file in data_files:
                if '_nli_data' not in input_file:
                    continue
                examples = self.read_json_files(os.path.join(self.config.data_path, input_file))
                print(len(examples),'heeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
        ...
        with open(file_path, "r", encoding='utf-8') as f:
            try:
                ...
                for linenum, line in enumerate(f):
                    ...
                    data_dict = {}
                    data_dict['query'] = sample[0]
                    data_dict['ans1'] = sample[2]
                    data_dict['ans2'] = sample[4]
                    if self.config.shuffle:
                        data_dict['label'] = sample[5]
                    else:
                        data_dict['label']='0'
    ...
    def get_num_examples(self):
        ...
        sum_examples = 0
        for input_file in data_files:
            if "answer_nli_data" in input_file:
                examples = self.read_json_files(os.path.join(self.config.data_path, input_file))
