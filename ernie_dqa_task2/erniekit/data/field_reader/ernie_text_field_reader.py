...
class ErnieTextFieldReader(BaseFieldReader):
    ...
    def convert_texts_to_ids(self, batch_text, use_random_pos=False, max_pos_id=2048):
        ...
        for sample in batch_text:
            ...
            tokens_ans1 = self.tokenizer.tokenize(ans1)
            tokens_ans2 = self.tokenizer.tokenize(ans2)

            # tokens_doc1=tokens_query+['#']+tokens_ans1
            # tokens_doc2=tokens_query+['#']+tokens_ans2
            tokens_doc=tokens_ans1+['[SEP]']+tokens_ans2
            # 加上截断策略
            # while len(tokens_doc2)+len(tokens_doc1) > self.field_config.max_seq_len - 3:
            #     if len(tokens_doc2)>len(tokens_doc1):
            #         tokens_doc2.pop()
            #     else:
            #         tokens_doc1.pop()
            ...
            # assert len(tokens_doc1)+len(tokens_doc2)<=self.field_config.max_seq_len - 3 
            sentence_id = []
            tokens = []
            tokens.append("[CLS]")
