...
class ErnieFcSeqLabel(BaseModel):
    ...
    def structure(self):
        ...
        self.fc_prediction_similary = paddle.nn.Linear(in_features=self.hid_dim, out_features=2,
        ...
        self.loss = paddle.nn.CrossEntropyLoss(use_softmax=False, reduction='mean')
    ...
    def forward(self, fields_dict, phase):
        ...
        cls_embedding, _ = self.ernie_model(src_ids=text_src, sent_ids=text_sent,
                                                           task_ids=text_task)

        emb_text = self.dropout(cls_embedding)

        # _, seq_embedding = self.ernie_model(src_ids=text_src, sent_ids=text_sent,
        #                                                    task_ids=text_task)
        # text_mask_seq=paddle.unsqueeze(text_mask,axis=-1)
        # seq_embedding=seq_embedding*text_mask_seq                                               
        # seq_embedding=paddle.max(seq_embedding, axis=1)

        # emb_text = self.dropout(seq_embedding)


        # [batch_size, max_seq_len] 
        logits_start = paddle.nn.functional.softmax(self.fc_prediction_similary(emb_text), axis=-1)
        ...
        loss=self.loss(logits_start,label_start)

        # probs_start = probs_start * text_mask
        # probs_end = probs_end * text_mask
        if phase == InstanceName.SAVE_INFERENCE:
            target_predict_list = [logits_start]
            target_feed_list = [text_src, text_sent]
