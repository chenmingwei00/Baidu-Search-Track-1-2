...
class CustomInference(BaseInference):
    ...
    def inference_batch(self):
        ...
        for batch_id, data in enumerate(dg()):
            ...
            # 回调给解析函数
              # 回调给解析函数
            write_result_list,write_probs = self.parser_handler(predict_results,
                    input_list=data_tokens, params_dict=self.params, seq_lens=seq_lens)
            for score,prob in zip(write_result_list,write_probs):
                output_file.write(str(score)+'\t'+str(prob)+'\n')
